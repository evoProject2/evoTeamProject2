...
import {setNeedFilterFlag, setSortingDirection, setSortingType} from "../../reducers/filterSlice";
...
export const FilterBar = () => {
    ...
    return filter.show && <div className={"filter-bar-component-container"}>
        ...
        <FormControl fullWidth>
            ...
            >
                {Object.keys(FILTER.sortBy).map(type => <MenuItem key={type}
                                                                  value={type}>{capitalize(FILTER.sortBy[type])}</MenuItem>)}
            </Select>
        </FormControl>
                ...
                >
                    {
                        Object.keys(FILTER.directions).map(direction =>
                            <FormControlLabel disabled={filter.sorting.type == 'none' ? true : false}
                                              key={direction} value={direction} control={<Radio/>}
